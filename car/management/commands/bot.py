...
#все имена объекта car, через фор делаем список(пока цифарки)
list_name_car = ['Lamborghini Huracan Spyder', 'Chevrolet Corvette',
                 'Ferrari California', 'Lamborghini Urus',
                 'Audi R8', 'Chevrolet Camaro',
                 'Maserati Quattroporte', 'Dodge Challenger',
                 'Nissan GT-R', 'Land Rover Range Rover',
                 'BMW X6'
                 ]
...
class Command(BaseCommand):
    ...
    def start(message):
        ...
        bot.send_message(message.from_user.id,
                         "🔎 Вы хотите перейти на сайт и узнать о компании / 🚗 хотите знакомится с каталогом автомобилей и оставить заявку",
                         reply_markup=markup)
    ...
    def get_text_messages(message):
        if message.text == '🔎 О компании':
            ...
            bot.send_message(message.from_user.id,
                             'Чтобы подробнее уpнать о компании переходите на сайт:\n 📲 Перейти по' + ' [ссылке](https://www.landrover.ru/vehicles/new-range-rover/index.html)',
            ...
            markup.add(buttons['about'], buttons['car_catalog'])
            bot.send_message(message.from_user.id,
                             "🔎 Вы хотите перейти на сайт и узнать о компании / 🚗 хотите ознакомится с каталогом автомобилей и оставить заявку", reply_markup=markup)
            #
            ...
            for i in range(1, 12):
                bot.send_message(message.from_user.id, f"машина песня {i}", reply_markup=markup)
                #photo = open(f'car/static/car/images/{i}.png', 'rb')
            ...
            markup.add(buttons['main_menu'])
            bot.send_message(message.from_user.id,
                             f"Отличный выбор 👍, чтобы оставить заявку на {message.text}, укажите следующие данные: \n Ваш номер телефона.", reply_markup=markup)
